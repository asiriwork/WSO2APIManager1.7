...
public class APIMWSDLReader {	
	...
	public OMElement readAndCleanWsdl(String transports) throws APIManagementException {

		try {
			Definition wsdlDefinition = readWSDLFile();

			setServiceDefinition(wsdlDefinition, transports);
	...
	private void setServiceDefinition(Definition definition, String transports) throws APIManagementException {
				...
				while (portItr.hasNext()) {
					...
					for (int i = 0; i < extensibilityElementList.size(); i++) {
						...
						if (addressURI == null) {
							...
							setAddressUrl(extensibilityElement, transports);
						}
					}
				}
			}
	...
	private void setAddressUrl(ExtensibilityElement exElement, String transports) throws APIManagementException {

		if (exElement instanceof SOAP12AddressImpl) {
			((SOAP12AddressImpl) exElement).setLocationURI(APIUtil.getGatewayendpoint(transports));
		} else if (exElement instanceof SOAPAddressImpl) {
			 ((SOAPAddressImpl) exElement).setLocationURI(APIUtil.getGatewayendpoint(transports));
		} else if (exElement instanceof HTTPAddressImpl) {
			 ((HTTPAddressImpl) exElement).setLocationURI(APIUtil.getGatewayendpoint(transports));
		} else {
			String msg = "Unsupported WSDL errors!";
			log.error(msg);
			throw new APIManagementException(msg);
